...
local folder_commands= require("folder_commands")
...
while flag==true do
    ::reset::
    io.write(ITALIC..BOLD..YELLOW .."\n $ Working Directory : "..cwd..RESET)
    io.write(ITALIC..BOLD..RED .. "\n # Beer Shell $> "..RESET)

    --reading user input 
    local raw_input = io.read()

    if raw_input == nil or raw_input == "" then
        print(MAGENTA .."No input entered. Please type a command."..RESET)
        goto reset
    end
    ...
    for word in string.gmatch(raw_input, "%w+") do   -- %w+ allows alpha numeric values
        ...
        if command_length>3 then 
            io.write(MAGENTA.."command length too long // only 3 words are allowed max"..RESET)
            goto reset
        end
    end
    ...
    elseif command=="folder" then
        folder_commands.folder_commands(command,arg1,arg2)
    elseif command_length==1 then
        basic_commands.basic_commands(command)
    else
        io.write(MAGENTA..""..raw_input.." <- command not found"..RESET);
    end
end
